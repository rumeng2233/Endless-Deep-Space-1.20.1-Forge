...
public class TrackingBowItem extends ModBowItem {
   public static final Predicate<ItemStack> ARROW_ONLY = (p_43017_) -> {
      return p_43017_.is(ModItems.TRACKING_ARROW.get());
   };
   
   public TrackingBowItem(Properties p_40660_) {
      super(p_40660_);
   }
      ...
      if (p_40669_ instanceof Player player) {
         ...
         if (!itemstack.isEmpty() || flag) {
            if (itemstack.isEmpty()) {
               itemstack = new ItemStack(ModItems.TRACKING_ARROW.get());
            }
            
            float f = getPowerForTime(i);
            if (!((double)f < 0.1D)) {
               boolean flag1 = player.getAbilities().instabuild || (itemstack.getItem() == ModItems.TRACKING_ARROW.get() && EnchantmentHelper.getItemEnchantmentLevel(Enchantments.INFINITY_ARROWS, p_40667_) > 0);
               if (!p_40668_.isClientSide) {
                  Arrow abstractarrow = new Arrow(p_40669_, p_40668_, Arrow.ArrowType.TRACKING_ARROW);
