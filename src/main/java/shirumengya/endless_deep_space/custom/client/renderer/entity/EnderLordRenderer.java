...
@OnlyIn(Dist.CLIENT)
public class EnderLordRenderer extends EntityRenderer<EnderLord> {
   public static final ResourceLocation CRYSTAL_BEAM_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/enderlord/dragon_beam.png");
   private static final ResourceLocation DRAGON_EXPLODING_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/enderlord/dragon_exploding.png");
   private static final ResourceLocation DRAGON_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/enderlord/dragon.png");
   private static final ResourceLocation DRAGON_EYES_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/enderlord/dragon_eyes.png");
   public static final ResourceLocation END_SKY_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/end_sky.png");
   public static final ResourceLocation END_PORTAL_LOCATION = new ResourceLocation("endless_deep_space:textures/entities/end_portal.png");
   ...
   private static final RenderType BEAM = RenderType.entitySmoothCutout(CRYSTAL_BEAM_LOCATION)/*ModRenderType.EnderLordDeath(END_SKY_LOCATION, END_PORTAL_LOCATION)*/;
   private static final float HALF_SQRT_3 = (float)(Math.sqrt(3.0D) / 2.0D);
   private final EnderLordRenderer.DragonModel model;

   public EnderLordRenderer(EntityRendererProvider.Context p_173973_) {
      super(p_173973_);
      this.shadowRadius = 0.5F;
      this.model = new EnderLordRenderer.DragonModel(p_173973_.bakeLayer(ModelLayers.ENDER_DRAGON));
   }
   
   @Override
   public boolean shouldRender(EnderLord p_114491_, Frustum p_114492_, double p_114493_, double p_114494_, double p_114495_) {
      return true;
   }
   
   public static java.awt.Color rainbow(float speed, float saturation, float lgiht) {
      float colorr = (float) ModMusicManager.getMillis() / speed % 1.0F;
      java.awt.Color color = java.awt.Color.getHSBColor(colorr, saturation, lgiht);
      return color;
   }

   public static Vector4f rainbowV4(float speed, float saturation, float lgiht) {
      float colorr = (float) ModMusicManager.getMillis() / speed % 1.0F;
      java.awt.Color color = java.awt.Color.getHSBColor(colorr, saturation, lgiht);
      return new Vector4f(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
   }

   protected int getBlockLightLevel(EnderLord p_114087_, BlockPos p_114088_) {
      return 15;
   }

   public void render(EnderLord p_114208_, float p_114209_, float p_114210_, PoseStack p_114211_, MultiBufferSource p_114212_, int p_114213_) {
      p_114211_.pushPose();
      ...
      ModRenderType.setTargetStack(new ItemStack(ModItems.ENDER_LORD_SPAWN_EGG.get()));
      if (p_114208_.dragonDeathTime > 0) {
         float f2 = (float)p_114208_.dragonDeathTime / 800.0F;
         //VertexConsumer vertexconsumer = p_114212_.getBuffer(RenderType.dragonExplosionAlpha(DRAGON_EXPLODING_LOCATION));
         VertexConsumer vertexconsumer = ItemRenderer.getFoilBufferDirect(p_114212_, RenderType.dragonExplosionAlpha(DRAGON_EXPLODING_LOCATION), false, true);
         ...
         VertexConsumer vertexconsumer3 = ItemRenderer.getFoilBufferDirect(p_114212_, RENDER_TYPE, false, true);
         //VertexConsumer vertexconsumer3 = p_114212_.getBuffer(RenderType.endGateway());
      ...
      if (p_114208_.dragonDeathTime > 0) {
         float f5 = ((float)p_114208_.dragonDeathTime + p_114210_) / 800.0F;
         ...
         p_114211_.scale(p_114208_.dragonDeathTime / 40.0F, p_114208_.dragonDeathTime / 40.0F, p_114208_.dragonDeathTime / 40.0F);
         ...
         for(int i = 0; (float)i < (f5 + f5 * f5) / 2.0F * 60.0F; ++i) {
            ...
            vertex01(vertexconsumer2, matrix4f, j, 85, 255, 255);
            vertex2(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
            vertex3(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
            vertex01(vertexconsumer2, matrix4f, j, 85, 255, 255);
            vertex3(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
            vertex4(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
            vertex01(vertexconsumer2, matrix4f, j, 85, 255, 255);
            vertex4(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
            vertex2(vertexconsumer2, matrix4f, f3, f4, 85, 255, 255);
         }
      ...
      if (p_114208_.chargingTime > 0) {
         java.awt.Color color = EnderLordRenderer.rainbow(3000.0F, 0.5F, 1.0F);
         float f5 = ((float)p_114208_.chargingTime + p_114210_) / 200.0F;
         float f7 = Math.min(f5 > 0.8F ? (f5 - 0.8F) / 0.2F : 0.0F, 1.0F);
         RandomSource randomsource = RandomSource.create(432L);
         VertexConsumer vertexconsumer2 = p_114212_.getBuffer(RenderType.lightning());
         p_114211_.pushPose();
         p_114211_.translate(0.0F, -1.0F, -2.0F);
         float scale = p_114208_.chargingTime / 10.0F;
         if (scale >= 10.0F) {
            scale = 20.0F - scale;
         }
         p_114211_.scale(scale, scale, scale);
         ...
         for(int i = 0; (float)i < (f5 + f5 * f5) / 2.0F * 60.0F; ++i) {
            ...
            Matrix4f matrix4f = p_114211_.last().pose();
            int j = (int)(255.0F * (1.0F - f7));
            vertex01(vertexconsumer2, matrix4f, j, color.getRed(), color.getGreen(), color.getBlue());
            vertex2(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
            vertex3(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
            vertex01(vertexconsumer2, matrix4f, j, color.getRed(), color.getGreen(), color.getBlue());
            vertex3(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
            vertex4(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
            vertex01(vertexconsumer2, matrix4f, j, color.getRed(), color.getGreen(), color.getBlue());
            vertex4(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
            vertex2(vertexconsumer2, matrix4f, f3, f4, color.getRed(), color.getGreen(), color.getBlue());
         }
      ...
      if (p_114208_.hasAttackTarget() && p_114208_.getActiveAttackTarget() != null && p_114208_.getActiveAttackTarget().isAlive()) {
         p_114211_.pushPose();
         float f6 = (float) (p_114208_.getActiveAttackTarget().getX() - Mth.lerp(p_114210_, p_114208_.xo, p_114208_.getX()));
         float f8 = (float) (p_114208_.getActiveAttackTarget().getY() - Mth.lerp(p_114210_, p_114208_.yo, p_114208_.getY()));
         float f9 = (float) (p_114208_.getActiveAttackTarget().getZ() - Mth.lerp(p_114210_, p_114208_.zo, p_114208_.getZ()));
         if (p_114208_.getActiveAttackTarget() instanceof Player) {
            if (ModClientConfig.ENDER_LORD_RENDER_BEAM_TO_PLAYER.get()) {
               renderCrystalBeams(f6, f8, f9, p_114210_, p_114208_.tickCount, p_114211_, p_114212_, p_114213_, BEAM, Color.of(0, 255, 255), Color.of(0, 255, 255));
               renderCrystalBeams(f6, f8, f9, p_114210_, p_114208_.tickCount, p_114211_, p_114212_, p_114213_, ModRenderType.EnderLordAttackTargetBeam(END_SKY_LOCATION, END_PORTAL_LOCATION, true), Color.of(0, 255, 255), Color.of(0, 255, 255));
            }
         } else {
            renderCrystalBeams(f6, f8, f9, p_114210_, p_114208_.tickCount, p_114211_, p_114212_, p_114213_, ModRenderType.EnderLordAttackTargetBeam(END_SKY_LOCATION, END_PORTAL_LOCATION, false), Color.of(0, 255, 255), Color.of(0, 255, 255));
         }
         p_114211_.popPose();
      }
      ...
      if (p_114208_.dragonDeathTime > 0) {
         p_114211_.pushPose();
         float size = p_114208_.dragonDeathTime / 4.0F;
         if (size >= 100.0F) {
            size = 200.0F - size;
         }
         p_114211_.translate(0.0F, -size + 1.0F, 0.0F);
         renderCube(-size, size, size, p_114211_.last().pose(), p_114212_.getBuffer(ModRenderType.EnderLordDeath(END_SKY_LOCATION, END_PORTAL_LOCATION, false)));
         p_114211_.popPose();

         List<Entity> _entfound = p_114208_.level().getEntitiesOfClass(Entity.class, new AABB(new Vec3(p_114208_.getX(), p_114208_.getY(), p_114208_.getZ()), new Vec3(p_114208_.getX(), p_114208_.getY(), p_114208_.getZ())).inflate(200 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(new Vec3(p_114208_.getX(), p_114208_.getY(), p_114208_.getZ())))).toList();
         for (Entity entityiterator : _entfound) {
            Entity entity = (Entity) entityiterator;
            p_114211_.pushPose();
            float f6 = (float) (entity.getX() - Mth.lerp(p_114210_, p_114208_.xo, p_114208_.getX()));
            float f8 = (float) (entity.getY() - Mth.lerp(p_114210_, p_114208_.yo, p_114208_.getY()));
            float f9 = (float) (entity.getZ() - Mth.lerp(p_114210_, p_114208_.zo, p_114208_.getZ()));
            if (entity instanceof Player) {
               if (ModClientConfig.ENDER_LORD_RENDER_BEAM_TO_PLAYER.get()) {
                  renderCrystalBeams(f6, f8, f9, p_114210_, p_114208_.tickCount, p_114211_, p_114212_, p_114213_, BEAM, Color.of(85, 255, 255), Color.of(45, 143, 137));
               }
            } else {
               renderCrystalBeams(f6, f8, f9, p_114210_, p_114208_.tickCount, p_114211_, p_114212_, p_114213_, BEAM, Color.of(85, 255, 255), Color.of(45, 143, 137));
            }
   ...
   public static void vertex01(VertexConsumer p_254498_, Matrix4f p_253891_, int p_254278_, int red, int green, int blue) {
      p_254498_.vertex(p_253891_, 0.0F, 0.0F, 0.0F).color(red, green, blue, p_254278_).endVertex();
   }

   public static void vertex2(VertexConsumer p_253956_, Matrix4f p_254053_, float p_253704_, float p_253701_, int red, int green, int blue) {
      p_253956_.vertex(p_254053_, -HALF_SQRT_3 * p_253701_, p_253704_, -0.5F * p_253701_).color(red, green, blue, 0).endVertex();
   }

   public static void vertex3(VertexConsumer p_253850_, Matrix4f p_254379_, float p_253729_, float p_254030_, int red, int green, int blue) {
      p_253850_.vertex(p_254379_, HALF_SQRT_3 * p_254030_, p_253729_, -0.5F * p_254030_).color(red, green, blue, 0).endVertex();
   }

   public static void vertex4(VertexConsumer p_254184_, Matrix4f p_254082_, float p_253649_, float p_253694_, int red, int green, int blue) {
      p_254184_.vertex(p_254082_, 0.0F, p_253649_, 1.0F * p_253694_).color(red, green, blue, 0).endVertex();
   }

   public static void renderCrystalBeams(float p_114188_, float p_114189_, float p_114190_, float p_114191_, int p_114192_, PoseStack p_114193_, MultiBufferSource p_114194_, int p_114195_, RenderType renderType, Color startColor, Color endColor) {
      ...
      VertexConsumer vertexconsumer = p_114194_.getBuffer(renderType);
      ...
      for(int j = 1; j <= 8; ++j) {
         ...
         vertexconsumer.vertex(matrix4f, f4 * 0.2F, f5 * 0.2F, 0.0F).color(startColor.red, startColor.green, startColor.blue, startColor.alpha).uv(f6, f2).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_114195_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         vertexconsumer.vertex(matrix4f, f4, f5, f1).color(endColor.red, endColor.green, endColor.blue, endColor.alpha).uv(f6, f3).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_114195_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         vertexconsumer.vertex(matrix4f, f7, f8, f1).color(endColor.red, endColor.green, endColor.blue, endColor.alpha).uv(f9, f3).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_114195_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         vertexconsumer.vertex(matrix4f, f7 * 0.2F, f8 * 0.2F, 0.0F).color(startColor.red, startColor.green, startColor.blue, startColor.alpha).uv(f9, f2).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_114195_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         f4 = f7;
         f5 = f8;
         f6 = f9;
      }
   ...
   public static void renderCube(float size1, float size2, float size3, Matrix4f p_173692_, VertexConsumer p_173693_) {
      renderFace(p_173692_, p_173693_, size2, size1, size2 + size3, size1 + size3, size1, size1, size1, size1, Direction.SOUTH);
      renderFace(p_173692_, p_173693_, size2, size1, size1 + size3, size2 + size3, size2, size2, size2, size2, Direction.NORTH);
      renderFace(p_173692_, p_173693_, size1, size1, size1 + size3, size2 + size3, size2, size1, size1, size2, Direction.EAST);
      renderFace(p_173692_, p_173693_, size2, size2, size2 + size3, size1 + size3, size2, size1, size1, size2, Direction.WEST);
      renderFace(p_173692_, p_173693_, size2, size1, size2 + size3, size2 + size3, size2, size2, size1, size1, Direction.DOWN);
      renderFace(p_173692_, p_173693_, size2, size1, size1 + size3, size1 + size3, size1, size1, size2, size2, Direction.UP);
   }

   public static void renderFace(Matrix4f p_254247_, VertexConsumer p_254390_, float p_254147_, float p_253639_, float p_254107_, float p_254109_, float p_254021_, float p_254458_, float p_254086_, float p_254310_, Direction p_253619_) {
   ...
   public ResourceLocation getTextureLocation(EnderLord p_114206_) {
   ...
   @OnlyIn(Dist.CLIENT)
   public static class DragonModel extends EntityModel<EnderLord> {
      ...
      public void prepareMobModel(EnderLord p_114269_, float p_114270_, float p_114271_, float p_114272_) {
         this.entity = p_114269_;
         this.a = p_114272_;
      }

      public void setupAnim(EnderLord p_114274_, float p_114275_, float p_114276_, float p_114277_, float p_114278_, float p_114279_) {
      }
